...
local function replace_bnescape(id)
    for index = 1, BNGetNumFriends() do
        local info = C_BattleNet.GetFriendAccountInfo(index);
        if info ~= nil and info.accountName == id then
            return (info.battleTag ~= nil and info.battleTag ~= "") and strsplit("#", info.battleTag) or "";
        end
    end
end
...
local newSetItemRef = function(link, text, button, ...)
    ...
    if(text) then
		...
		text = text:gsub('|T.-|t', '')
		text = text:gsub('|K.-|k', replace_bnescape)

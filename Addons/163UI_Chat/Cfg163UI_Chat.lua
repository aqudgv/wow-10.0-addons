...
U1RegisterAddon("163UI_Chat", {
    ...
    {
        var="short_channel_name",
        default = true,
        text="短频道名",
        tip="开启后，聊天窗口中的频道名，将被缩写为一个字",
        callback = function(cfg, v, loading)
            if v then
                _163UI_Chat_ChatFrame.short_channel_name.start();
            else
                _163UI_Chat_ChatFrame.short_channel_name.stop();
            end
        end,
    },
    {
        var="short_realm_name",
        default = false,
        text="聊天窗口中缩写服务器名",
        tip="开启后，聊天窗口中的角色名，后面的服务器将被简写为一个字，例如\"格瑞姆巴托\"缩写为\"格\"",
        callback = function(cfg, v, loading)
            if v then
                _163UI_Chat_ChatFrame.short_realm_name.start();
            else
                _163UI_Chat_ChatFrame.short_realm_name.stop();
            end
        end,
    },
    {
    ...
    },
    {
        ...
        options = {
            "无", nil,
            "分:秒", "%M:%S", "时:分", "%H:%M", "时分秒", "%H:%M:%S",
            "时:分(12小时)", "%I:%M", "时分秒(12小时)", "%I:%M:%S"
        },
        cols = 2,
        callback = function(cfg, v, loading)
            if v then
                local old = GetCVar("showTimestamps");
                if old ~= "none" then U1DBA.showTimestamps = old end
                SetCVar("showTimestamps", "none");
            else
                if U1DBA.showTimestamps then
                    SetCVar("showTimestamps", U1DBA.showTimestamps)
                    U1DBA.showTimestamps = nil;
                end
            end
            if(v == '无' or v == '') then
    ...
    },
    {
        ...
        callback = function(cfg, v, loading)
            if loading then
                _Store["ChatEdit_CustomTabPressed_Inner"] = _G["ChatEdit_CustomTabPressed_Inner"];
            else
                if v then
                    _G["ChatEdit_CustomTabPressed_Inner"] = _Store["ChatEdit_CustomTabPressed_Inner"];
                elseif _Store["ChatEdit_CustomTabPressed_Inner"] then
                    _G["ChatEdit_CustomTabPressed_Inner"] = noop;
                end
            end
        end,
        {

...
function addon:GetUnitBuffTimer(unit, buff, mine)
	if unit and buff then
		local filter = mine and "PLAYER" or nil;
		for index = 1, 40 do
			local name, _, _, count, _, expires, _, _, _, id = UnitBuff(unit, index, filter)
			if not name then
				return
			end
			if name == buff then
			...
			if __map[buff] == id then	---buff == GetSpellInfo(315584) and id == 2823 then
				return expires or 0, count or 1
			end
		end
	end

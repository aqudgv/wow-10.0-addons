...
local function saveInfoBeforeDeath(destName, sourceName, skill, amount, overkill, resisted, blocked, absorbed)
    ...
    if avoid > 0 then
        amount = amount + avoid
        more = "(含吸收|cffffff00" .. AbbreviateNumbers(avoid) .. "|r"
    end
    if overkill > 0 then
        amount = amount --+ overkill 好像不应该算
        more = (more and more .. "," or "(含") .. "过量|cffff0000" .. AbbreviateNumbers(overkill) .. "|r"
    end
    more = more and more .. ")" or ""
    DEATH_ANNOUNCE_OVK[destName] = (overkill > 0 and "-" or " ").."("..(sourceName and sourceName..":" or "")..(skill and skill.."/" or "").."|cffff0000"..(type(amount)=="number" and AbbreviateNumbers(amount) or amount).."|r)"..more;
end
...
function DEATH_ANNOUNCE_CLEU(...)
    ...
    if ( subevent == "SPELL_INTERRUPT" ) then
        ...
        if sourceGUID == playerGUID and U1GetCfgValue and U1GetCfgValue("deathannounce/yell") then
            SendMessage(string.format("我已打断%s的%s", destName, GetSpellLink(param4)))
        else
        ...
        if DEATH_ANNOUNCE_IMPORTANT_SPELLS[spellId] then
            if sourceGUID == playerGUID and U1GetCfgValue and U1GetCfgValue("deathannounce/yell_spell") then
                if destGUID ~= playerGUID then
                    SendMessage(string.format("%s -> %s", GetSpellLink(spellId), destName))
                end
            else
...
function DEATH_ANNOUNCE_Print(msg, announce, link, deadName)
    ...
    if not announce then
        ...
        if (announce=="FORCE" or DEATH_ANNOUNCE_IsEnabled()) and IsInGroup() then
            ...
            SendChatMessage(advised and msg or ("【网.易.有.爱】"..msg), channel);
            advised = true
            if(ChatFrame2) then

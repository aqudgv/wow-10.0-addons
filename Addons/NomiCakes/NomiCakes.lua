...
local function DecorateNomi()
	...
	local GossipTitleButtons = GossipFrame.buttons;--GossipFrame.titleButtonPool.activeObjects;
	local i = 0
	for j = 1, #IngredientOrder do
		...
		if count >= 5 then -- we have enough of an ingredient for nomi to display it
			...
			local button = GossipTitleButtons[i];
			local buttonIcon = button.Icon;
			if button and button:IsShown() and buttonIcon and
				(buttonIcon:GetTexture():lower() == 'interface\\gossipframe\\workordergossipicon' or
				buttonIcon:GetTexture():lower() == 'interface/gossipframe/workordergossipicon')
				then
				...
				if ingredient and recipeList then -- and recipes then
					...
					button:Resize();-- GossipResize(button)
				end
			else
				break
			end

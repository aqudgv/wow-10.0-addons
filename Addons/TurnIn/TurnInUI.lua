...
function TI_StatusIndicatorUpdate()
	if(TI_status) then
		if(TI_status.state) then
			TI_StatusIndicator_Status:SetText("开");
			TI_StatusIndicator_Status:SetTextColor(0,1,0);
		else
			TI_StatusIndicator_Status:SetText("关");
...
function TI_NPCListCheckboxTooltip(self)
	if(self:GetChecked()) then
		TI_TooltipMessage(self, "NPC 使用特殊的设置.");
	else
		TI_TooltipMessage(self, "NPC 使用默认的设置.");
	end
end

function TI_OptionListCheckboxTooltip(self)
	if(self:GetChecked()) then
		TI_TooltipMessage(self, "这个选项已启用.");
	else
		TI_TooltipMessage(self, "这个选项已禁用.");
	end
end

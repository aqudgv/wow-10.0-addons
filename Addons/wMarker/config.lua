...
local function clamp()
	wMarker.main:SetClampedToScreen(wMarkerDB.clamped)
	wFlares.main:SetClampedToScreen(wFlaresDB.clamped)
end
...
event:SetScript("OnEvent", function(self,event,addon,...)
	...
	elseif (event=="GROUP_ROSTER_UPDATE") or (event=="RAID_ROSTER_UPDATE") or (event=="PLAYER_TARGET_CHANGED") then
		wMarker:visibility()
...
local has_load;
function bf_wMarker_Toggle(toggle)
	if toggle then

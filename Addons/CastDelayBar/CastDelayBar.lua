...
function C:OnInitialize()	
	...
	self.delayBar = PlayerCastingBarFrame:CreateTexture("StatusBar", "BACKGROUND");
	self.delayBar:SetHeight(PlayerCastingBarFrame:GetHeight());
...
function C:ShowDelayBar(maxValue)
    if (self.enable and self.timeDiff) then
        ...
        self.delayBar:SetWidth(PlayerCastingBarFrame:GetWidth() * modulus);
        self.delayBar:ClearAllPoints()
        if (self.casting) then
            self.delayBar:SetPoint("RIGHT", PlayerCastingBarFrame, "RIGHT",  0, 0);
        elseif (self.channeling) then
            self.delayBar:SetPoint("LEFT", PlayerCastingBarFrame, "LEFT",  0, 0);
        end
        self.delayBar:Show();
    else
        self.delayBar:Hide()

...
if _toc_version >= 70000 then			--	MountTip
	...
	local function _Init()
		...
		UpdateMountsData();
		hooksecurefunc(TargetFrame, "UpdateAuras", HookTargetFrame_UpdateAuras);
	end
...
if false then										--	BuffFrame & TemporaryEnchantFrame
	local _enabled = false;
	local _FontSize = 10;
...
--										--	LargeAura
	...
	local LARGE_AURA_SIZE = 21;	--	TargetFrame.lua
	local SMALL_AURA_SIZE = 17;

	local function HookTargetFrame_UpdateBuffAnchor(self, buff, index, numBuffs, anchorBuff, anchorIndex, size, offsetX, offsetY, mirrorVertically)
	...
	local function HookTargetFrame_UpdateDebuffAnchor(self, buff, index, numBuffs, anchorBuff, anchorIndex, size, offsetX, offsetY, mirrorVertically)
		if _enabled_Debuff then
			if size == LARGE_AURA_SIZE then
				size = _LargeDebuffSize;
			else
				size = _SmallDebuffSize;
			end
			buff:SetSize(size, size);
		end
	end
	...
	local function _Init()
		_isInitialized = true;
		hooksecurefunc("TargetFrame_UpdateBuffAnchor", HookTargetFrame_UpdateBuffAnchor);
		hooksecurefunc("TargetFrame_UpdateDebuffAnchor", HookTargetFrame_UpdateDebuffAnchor);
	end
...
--										--	Cooldown
	...
	local function HookTargetFrame_UpdateAuraFrames(frame)
		if OmniCC and OmniCC.Cooldown and OmniCC.Cooldown.SetNoCooldownCount then
			for aura in frame.auraPools:GetPool("TargetDebuffFrameTemplate"):EnumerateActive() do
				OmniCC.Cooldown.SetNoCooldownCount(aura.Cooldown, _enabled_Buff);
			end
			for aura in frame.auraPools:GetPool("TargetBuffFrameTemplate"):EnumerateActive() do
				OmniCC.Cooldown.SetNoCooldownCount(aura.Cooldown, _enabled_Debuff);
			end
		end
	end
	...
	local function _Init()
		_isInitialized = true;
		hooksecurefunc(TargetFrame, "UpdateAuraFrames", HookTargetFrame_UpdateAuraFrames);
	end
	...
	local function _Enable_Buff(loading)
		...
		_enabled_Buff = true;
		HookTargetFrame_UpdateAuraFrames(TargetFrame);
	end
	local function _Disable_Buff(loading)
		_enabled_Buff = false;
		if _isInitialized then

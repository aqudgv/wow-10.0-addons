	...
	for player in BagnonDB:GetPlayers() do
		...
		if player == currentPlayer then
			...
			for i = 1, NUM_BANKBAGSLOTS do
				bankCount = bankCount + BagnonDB:GetItemCount(link, NUM_TOTAL_EQUIPPED_BAG_SLOTS + i, player)
			end
			...
			infoString = itemInfo and itemInfo[player] and itemInfo[player][link]
		end
...
TooltipDataProcessor.AddTooltipPostCall(Enum.TooltipDataType.Item, function(self, tooltipData)
	local link;
	if tooltipData.guid then
		link = C_Item.GetItemLinkByGUID(tooltipData.guid)

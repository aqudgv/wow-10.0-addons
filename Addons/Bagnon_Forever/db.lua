...
function BagnonDB:BAG_UPDATE(event, bag)
	-- if (bag > BACKPACK_CONTAINER and bag < NUM_TOTAL_EQUIPPED_BAG_SLOTS) or self.atBank then
		self:UpdateBag(bag)
	-- end
end
...
function BagnonDB:BANKFRAME_OPENED()
	...
	for i = 1, GetNumBankSlots() do
		self:UpdateBag(i + NUM_TOTAL_EQUIPPED_BAG_SLOTS)
	end
end
...
function BagnonDB:GetItemCount(itemLink, bag, player)
	local total = 0
	local itemLink = select(2, GetItemInfo(ToShortLink(itemLink)))		-- what's the fuck??
...
function BagnonDB:SaveBag(bag)
	...
	if size > 0 then
		local equipSlot = bag > 0 and C_Container.ContainerIDToInventoryID(bag)

...
local ignored = {
	...
	["ExpansionLandingPageMinimapButton"] = true,	--10.0
}
...
do
	BigFoot_DelayCall(function()
		if ExpansionLandingPageMinimapButton then
			ExpansionLandingPageMinimapButton:RegisterForClicks("LeftButtonDown","RightButtonDown");		--注册左右键点击事件
			if not BigFoot_SysTemSetTab.hideGarrisonMinimapButton then		--按默认的打开
				M:addignored("ExpansionLandingPageMinimapButton");
			else
				M:removeignored("ExpansionLandingPageMinimapButton");
			end
			if ExpansionLandingPageMinimapButton:HasScript("OnClick") then
				local script = ExpansionLandingPageMinimapButton:GetScript("OnClick");
				ExpansionLandingPageMinimapButton:SetScript("OnClick", function(self,...)
					local button = ...
					if button == "RightButton" then		--按右键则变动图标位置
						BigFoot_SysTemSetTab.hideGarrisonMinimapButton = not BigFoot_SysTemSetTab.hideGarrisonMinimapButton;
						if not BigFoot_SysTemSetTab.hideGarrisonMinimapButton then
							M:addignored("ExpansionLandingPageMinimapButton");
						else
							M:removeignored("ExpansionLandingPageMinimapButton");
						end

...
-------------------------------------------------------------------------
-- 初始化DropDown,一般用于OnLoad时
-- 参数:
--	frame: 保存信息的frame,继承至BDropDownMenuTemplate 缺省为this
--	initFunction: 函数方法
--	displayerMod: 显示方式,菜单还是..
--	level: 显示层次,缺省为 1
-------------------------------------------------------------------------
function BDropDownMenu_Initialize(frame, initFunction, displayMode, level, menuList)
	...
	if ( frame.displayMode ~= "MENU" and displayMode == "MENU" ) then
		...
		getglobal(frame:GetName().."Button"):SetPoint("RIGHT", _G[frame:GetName().."Text"], "RIGHT", 6, 0);

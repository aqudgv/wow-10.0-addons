--[[
	Player.lua
	实例化 模板并装载到TargetFrame上
	为其他子模块提供母模块
	Terry@bf
	2010 - 9 -30
]]

local T = LibStub("AceAddon-3.0"):GetAddon("TUnitFrame")
if not T then return end

local M = T:GetModule("Target",true) or T:NewModule("Target","AceHook-3.0")
if not M then return end

M.config = {
	self = 23,
	other = 17,
	row = 3,
	rowWidth = 125
}

local __3DPor = T.Templates and T.Templates.ThreeDportrait and T.Templates.ThreeDportrait:new()
local __ClassIcon = T.Templates and T.Templates.ClassIcon and T.Templates.ClassIcon:new()
local config = M.config

function M:Toggle3DPor(flag)
	if flag then
		__3DPor:SetPoint("TOPRIGHT", TargetFrame, "TOPRIGHT", -26, -19)
		__3DPor:Enable()
	else
		__3DPor:Disable()
	end
end

function M:ToggleClass(flag)
	if flag then
		__ClassIcon:Enable()
	else
		__ClassIcon:Disable()
	end
end

-- local LARGE_AURA_SIZE = 21; --MUST be the same as FrameXML/TargetFrame.lua	function ShouldAuraBeLarge(caster)
local PLAYER_UNITS = {
	player = true,
	vehicle = true,
	pet = true,
};

function M:TargetFrame_UpdateBuffAnchor(self, buff, index, numBuffs, anchorBuff, anchorIndex, size)
	local aura = C_UnitAuras.GetAuraDataByAuraInstanceID("target", buff.auraInstanceID)
	if PLAYER_UNITS[aura.sourceUnit] then
		size = config.self
	else
		size = config.other
	end
	buff:SetWidth(size);
	buff:SetHeight(size);
end

function M:TargetFrame_UpdateDebuffAnchor(self, buff, index, numBuffs, anchorBuff, anchorIndex, size)
	local aura = C_UnitAuras.GetAuraDataByAuraInstanceID("target", buff.auraInstanceID)
	if not aura then return end
	if PLAYER_UNITS[aura.sourceUnit] then
		size = config.self
	else
		size = config.other
	end
	buff:SetWidth(size);
	buff:SetHeight(size);
	local debuffFrame = buff.Border;
	debuffFrame:SetWidth(size+2);
	debuffFrame:SetHeight(size+2);
end

function M:OnInitialize()
	assert(__3DPor,"Target 3D template does not exist.")
	__3DPor:Create("Target",TargetFrame,TargetFrame.TargetFrameContainer.Portrait)

	assert(__ClassIcon,"Target class icon template does not exist.")
	__ClassIcon:Create(TargetFrame,"target")
	__ClassIcon:SetPoint("TOPLEFT",TargetFrame,"TOPLEFT",131,2)

	T:Debug("Target Module Initialized")
end

function M:OnEnable()
	self:SecureHook("TargetFrame_UpdateBuffAnchor")
	self:SecureHook("TargetFrame_UpdateDebuffAnchor")
	T:Debug("Target Module Enabled")
end

function M:OnDisable()
	self:UnhookAll()
	T:Debug("Target Module Disabled")
end

...
frame:SetScript("OnEvent", function(self, event)
    if(event == "VARIABLES_LOADED") then
        if U1DBA.configs[CONFIG] == nil then U1DBA.configs[CONFIG] = true end
        QuestWatchSortCheckButton:SetChecked(U1DBA.configs[CONFIG])
        EnableOrDisable()
    else
...
checkbox:SetScript("OnClick", function(self, button)
    U1DBA.configs[CONFIG] = self:GetChecked()
    EnableOrDisable()
end)

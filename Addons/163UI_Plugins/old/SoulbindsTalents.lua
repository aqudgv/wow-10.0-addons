...
function SoulbindsTalents:SetupOptions()
	self.options = {
		...
		args = {
			...
			version = {
				type = "description",
				name = "|cffffd100Version: 1.0.7|r",-- .. GetAddOnMetadata("SoulbindsTalents", "Version"),
				order = 2
			},
			title = {
				type = "description",
...
function SoulbindsTalents:ConduitTooltip_Rank(tooltip, rank)
	...
	for i = 3, 5 do
		if _G[tooltip:GetName() .. "TextLeft" .. i] then
			...
			if (level) then
				line:SetFormattedText("%s (等级 %d)", text, rank);
				return ;
			end
		end
	end
...
function SoulbindsTalents:ConduitRank(conduit, conduitData)
	...
	local itemCallback = function()
		...
		conduit.ItemLevel:SetFormattedText("%s (等级 %d)", conduitData.conduitItemLevel, conduitData.conduitRank);

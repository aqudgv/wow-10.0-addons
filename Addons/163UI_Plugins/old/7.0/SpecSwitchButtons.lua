...
if TalentMicroButton then
    ...
    local function showSwitchButtons()
        if InCombatLockdown() or U1DBA.configs[CONFIG_NAME] then return end
    ...
    SetOrHookScript(TalentMicroButton, "OnClick", function(self, button)
        ...
        elseif (IsControlKeyDown()) then
            U1DBA.configs[CONFIG_NAME] = not U1DBA.configs[CONFIG_NAME]
            U1Message("切换天赋按钮已 "..(U1DBA.configs[CONFIG_NAME] and "禁用" or "启用"))
            if PlayerTalentFrame:IsVisible() then
                if not InCombatLockdown() then
                    HideUIPanel(PlayerTalentFrame)
                end

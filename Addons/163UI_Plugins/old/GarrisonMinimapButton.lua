...
local ProcessGarrisonLandingPageMMB = function()
    ...
    local glpmmb = GarrisonLandingPageMinimapButton or ExpansionLandingPageMinimapButton
    glpmmb:Hide();
    glpmmb:SetAlpha(0.0);
    glpmmb:EnableMouse(false);
    ...
    U1DBA.configs.landingGarrisonMMB = U1DBA.configs.landingGarrisonMMB or {}
    local db = U1DBA.configs.landingGarrisonMMB
    ...
    --其他动画不用设置
    (GarrisonLandingPageMinimapButton or ExpansionLandingPageMinimapButton).LoopingGlow:SetSize(36,36)
    --关闭教程位置漂移
    if GarrisonLandingPageTutorialBox ~= nil then

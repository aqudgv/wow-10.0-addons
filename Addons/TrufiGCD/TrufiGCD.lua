...
local TrGCDBufferIcon = {} --счетчик расстояния между иконками
...
local TrGCDCastSp = {} -- 0 - каст идет, 1 - каст прошел и не идет
local TrGCDCastSpBanTime = {} --время остановки каста
local TrGCDBL = {} -- черный список спеллов
local BLSpSel = nil --выделенный спелл в блэклисте
local InnerBL = {
	[61391] = true, -- Тайфун x2
	[5374] = true, -- Расправа х3
	[27576] = true, -- Расправа (левая рука) х3
	[88263] = true, -- Молот Праведника х3
	[98057] = true, -- Великий воин Света
	[32175] = true, -- Удар бури
	[32176] = true, -- Удар бури (левая рука)
	[96103] = true, -- Яростный выпад
	[85384] = true, -- Яростный выпад (левая рука)
	[57794] = true, -- Героический прыжок
	[52174] = true, -- Героический прыжок
	[135299] = true, -- Ледяная ловушка
	[121473] = true, -- Теневой клинок
	[121474] = true, -- Второй теневой клинок
	[114093] = true, -- Хлещущий ветер (левая рука)
	[114089] = true, -- Хлещущий ветер
	[115357] = true, -- Свирепость бури
	[115360] = true, -- Свирепость бури (левая рука)
	[127797] = true, -- Вихрь урсола
	[102794] = true, -- Вихрь урсола
	[50622] = true, -- Вихрь клинков
	[122128] = true, -- Божественная звезда (шп)
	[110745] = true, -- Божественная звезда (не шп)
	[120696] = true, -- Сияние (шп)
	[120692] = true, -- Сияние (не шп)
	[115464] = true, -- Целительная сфера
	[126526] = true, -- Целительная сфера
	[132951] = true, -- Осветительная ракета
	[107270] = true, -- Танцующий журавль
	[137584] = true, -- Бросок сюрикена
	[137585] = true, -- Бросок сюрикена левой рукой
	[117993] = true, -- Ци-полет (дамаг)
	[124040] = true, -- Ци-полет (хил)
	[198928] = true, -- Cinderstorm shards (Fire Mage verified fix)
	[84721] = true, -- Frozen Orb shards (Frost Mage verified fix)
	[222031] = true, -- Chaos Strike 1 (DemonHunter unverified fix)
	[197125] = true, -- Chaos Strike 2 (DemonHunter unverified fix)
	[199547] = true, -- Chaos Strike 3 (DemonHunter unverified fix)
	[227255] = true, -- Spirit Bomb periodical
	[225919] = true, -- Fracture double hit
	[225921] = true, -- Fracture part 2
	[228478] = true, -- Soul Cleave part 2
	[346665] = true, -- Master of the Glaive (DH Class Tree Talent)
	[370966] = true, -- The Hunt Impact (DH Class Tree Talent)
	[394007] = true, -- Ready to Build (DF Engineering Accessoire)
	[391775] = true, -- What's Cookin'] = true, Good Lookin'? (DF Cooking Accessoire)
}
local TrGCDBLHash = {  };
...
local TrGCDSpStop = {} -- номер иконки у которой стопнулся каст спелла
local TrGCDSpStopTime = {} -- номер иконки у которой стопнулся каст спелла
local TrGCDSpStopName = {}
local TrGCDEnable = true
local PlayerDislocation = 0 -- Расположение игрока: 1 - Мир, 2 - ПвЕ, 3 - Арена, 4 - Бг.
local TrGCDIconOnEnter = {} -- false - курсор на иконке
local TrGCDTimeuseSpamSpell = {} -- время когда использовался спамящий в очередь спелл N -> SpellID -> Time

local TrGCDQueueOpt = {}
local TrGCDGUI, TrGCDFixEnable, TrGCDEnterEventFrame, TrGCDEventFrame, TrGCDEventBuffFrame
local TrGCDQueueFr = {}
local TrGCDIcon = {}
local TrGCDi = {} --счетчик TrGCDIcons
local TrGCDQueueFirst = {} -- очередь спеллов на первое место
local TrGCDQueueFirstI = {} --начало очереди, потом сдвигается, как спелл проходит в TrGCDQueueFr
...
local function TrGCDCheckToEnableAddon(t) -- проверяет галки EnableIn и от этого уже включен ли аддон
...
local function TrGCDLoadBlackList() -- загрузка черного списка
	for i=1,60 do
		if (TrGCDBL[i] ~= nil) then
			local spellname = GetSpellInfo(TrGCDBL[i])
			if (spellname == nil) then spellname = TrGCDBL[i] end
			TrGCDBLHash[spellname] = true;
			TrGCDBLHash[TrGCDBL[i]] = true;
...
local function TrGCDBLAddSpell(self)
...
local function TrGCDResizeQFr(i) -- ресайз после изменения размера очереди TrGCDQueueFr
...
local function TrGCDCheckEnableClick(i) --произошел клик по галочки вкл/выкл фреймов
...
local function TrGCDSizeQueue(i) -- узнать длину очереди спеллов на новое место
	return (#TrGCDQueueFirst[i] - TrGCDQueueFirstI[i])
end
local function TrGCDPlayerTarFocDetect(k) -- чек есть ли цель или фокус уже во фреймах (пати или арена)
...
local function TrGCDAddSpQueue(TrGCDit, i) -- добавить новый спелл на очередь спеллов на новое место
	TrGCDQueueFirst[i][#TrGCDQueueFirst[i] + 1] = TrGCDit
end
local unit_list = {
	player = 1,
	party1 = 2,
...
local function TrGCDEventBuffHandler(self,event, ...) --запущена эвентом изменения баффов/дебаффов персонажа
	if (TrGCDEnable) then
		local who = ... ;
		local i = unit_list[who]
		local tt = true
		if i ~= nil then
			for k=1,16 do
...
local function TrGCDUnitChannelInfo(unit)
...
local prev = -1;
local previcon = -1;
-- local multi_trigger_blacklist = {
-- 	[126664] = 100,		--	charge
-- 	[44949] = 190411,	--	wildwind	--	fury
-- 	[199667] = 190411,	--	wildwind	--	fury
-- 	[199658] = 1680,	--	wildwind	--	arms & prot
-- 	[184709] = 184367,	--	rampage		--	fury
-- 	[201363] = 184367,	--	rampage		--	fury
-- }


--[=[ main processor ]=]
local function TrGCDEventHandler(self, event, who, _, spellId)
	local i = unit_list[who] -- i - номер пользователя, t = true - если кто то из пати или на арене
	if (TrGCDEnable and i and TrGCDQueueOpt[i].enable) then
		local spellname, _, spellicon, casttime = GetSpellInfo(spellId);
		--print(spellId .. " - " .. spellname)
		local now = GetTime();
		TrGCDInsSp["time"][i] = now
		if (spellicon ~= nil and TrGCDBLHash[spellname] == nil and TrGCDBLHash[spellId] == nil and InnerBL[spellId] == nil and (now - prev > 0.001 or previcon ~= spellicon)) then
			...
			if (event == "UNIT_SPELLCAST_START") then
				...
				TrGCDAddGcdSpell(spellicon, i, spellId);prev = now;previcon=spellicon;
				TrGCDCastSp[i] = 0-- 0 - каст идет, 1 - каст прошел и не идет
				TrGCDCastSpBanTime[i] = now
			...
			elseif (event == "UNIT_SPELLCAST_SUCCEEDED") then
				if (TrGCDCastSp[i] == 0) then
					...
					elseif ((TrGCDInsSp["spell"][i] == 124430) and (spellId == 8092)) then b = true
					end
					TrGCDCastSpBanTime[i] = now
					if (IsChannel ~= nil) then TrGCDCastSp[i] = 0 end
					if (((now-TrGCDSpStopTime[i]) < 1) and (TrGCDSpStopName[i] == spellname) and (b == false)) then
...
local function TrGCDUpdate(self)
...
local function TrGCDUploadViewSetting()
...
local function TrGCDRestoreDefaultSettings() -- восстановление стандартных настроек
	...
	for i=1,12 do
		...
		TrGCDQueueOpt[i] = {}
		TrGCDQueueOpt[i].x = -200 - 120 * floor(i / 6)
		TrGCDQueueOpt[i].y = 256 + ((i - 1) % 5) * 32
		TrGCDQueueOpt[i].point = "CENTER"
		TrGCDQueueOpt[i].enable = i == unit_list.player
	...
	TrGCDQueueOpt[11].x = -200
	TrGCDQueueOpt[11].y = 256 - 32
	TrGCDQueueOpt[12].x = -200
	TrGCDQueueOpt[12].y = 256 - 64
...
local function TrGCDBLSaveSetting()
...
local function TrGCDBLLoadSetting()
...
local function TrGCDBLDefaultSetting()
	if (TrufiGCDChSave == nil) then TrufiGCDChSave = {} end
...
local function TrGCDSaveSettings()
	if (TrufiGCDGlSave == nil) then TrufiGCDGlSave = {} end
...
local function TrGCDLoadSettings()
...
local function TrGCDSpSizeChanged(i,value) --изменен размер иконок спеллов
...
local function TrGCDSpWidthChanged(i,value) --изменена длина очереди спеллов
	value = math.ceil(value);
...
local function TrGCDFadeMenuWasCheck(i, str) --выбрана строчка в меню направления фейда абилок
	TrGCDClear(i)
...
local function TrGCDGUIButtonFixClick() --функция кнопки show/hide в опциях
...
local function TrufiGCDAddonLoaded(self, event, ...)
	local arg1 = ...;

...
if _TocVersion > 70000 then
	...
	local function _LF_Hook_MountJournal_UpdateMountList()
		...
		local _MountButtons = MountJournal and MountJournal.ScrollBox and MountJournal.ScrollBox.view.frames
		for _index = 1, #_MountButtons do
			_LT_Externs.mount[_index] = _LT_Externs.mount[_index] or _LF_CreateExtern('mount', _MountButtons[_index]);
		end
	end
	...
	CoreDependCall(
		...
		function()
			...
			if MountJournalListScrollFrame then
				hooksecurefunc(MountJournalListScrollFrame, "update", _LF_Hook_MountJournalListScrollFrame_Update);
			else
				local MountJournalListScrollFrame = MountJournal and MountJournal.ScrollBox;
				if MountJournalListScrollFrame then
					hooksecurefunc(MountJournalListScrollFrame, "Update", _LF_Hook_MountJournalListScrollFrame_Update);
				end
			end
			...
			--	Pet
			local _PetButtons = (PetJournalListScrollFrame and PetJournalListScrollFrame.buttons) or (PetJournal and PetJournal.ScrollBox and PetJournal.ScrollBox.view.frames);
			_LT_Externs.pet = {  };
			...
			if PetJournalListScrollFrame then
				...
				local PetJournalListScrollFrame = PetJournal and PetJournal.ScrollBox;
				if PetJournalListScrollFrame then
					hooksecurefunc(PetJournalListScrollFrame, "Update", _LF_Hook_PetJournalListScrollFrame_Update);
				end
			end
...
if _TocVersion > 99930000 then
	...
	CoreDependCall(
		...
		function()
			...
			local _achbuttons = AchievementFrameAchievements.ScrollBox.view.frames;
			...
			hooksecurefunc("AchievementFrameAchievements_OnAchievementEarned", _LF_Hook_AchievementFrameAchievements_Update);
			hooksecurefunc("AchievementFrame_UpdateSearch", _LF_Hook_AchievementFrameAchievements_Update);
			hooksecurefunc(AchievementFrameAchievements.ScrollBox, "Update", _LF_Hook_AchievementFrameAchievements_Update);

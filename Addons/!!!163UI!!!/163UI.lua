...
do
    ...
    local hookStopMovingOrSizing = function(self)
        ...
        if self:GetLeft()==nil or self:GetTop()==nil or self:GetWidth()==nil or self:GetHeight()==nil then
            print(name)
            U1DBG.__vvv = U1DBG.__vvv or {}
            tinsert(U1DBG.__vvv, name)
        end
        local v1, v2, v3, v4 = self:GetLeft(), self:GetTop(), self:GetWidth(), self:GetHeight()
        if v1 and v2 and v3 and v4 then
            pos[1], pos[2], pos[3], pos[4] = v1*scale, v2*scale, v3, v4
        end

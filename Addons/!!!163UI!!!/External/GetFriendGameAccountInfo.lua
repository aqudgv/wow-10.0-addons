...
--	author 上官晓雾
--	https://ngabbs.com/read.php?tid=27432996
--	2021-09-15
--	9.1 组队邀请 战网好友 "不同的地区" 问题
...
local function FriendTipOnEnter(self)
	if self.buttonType == FRIENDS_BUTTON_TYPE_BNET then
		...
		if accountInfo and accountInfo.gameAccountInfo.gameAccountID and not accountInfo.gameAccountInfo.isInCurrentRegion and
			not _ShowRichPresenceOnly(
		...
		then
			...
			FriendsFrameTooltip_SetLine(
				...
				nil,
				BNET_FRIEND_TOOLTIP_ZONE_AND_REALM:format(areaName, realmName),
				-4
			)
		end
	else
...
if (FriendsListFrameScrollFrame.buttons) then
	for i = 1, #FriendsListFrameScrollFrame.buttons do
		FriendsListFrameScrollFrame.buttons[i].travelPassButton:HookScript(
			"OnEnter",
			function(self)
		...
		FriendsListFrameScrollFrame.buttons[i]:HookScript(
			"OnEnter",
			function(self)

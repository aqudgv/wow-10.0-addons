...
WW:Frame("MinimapZoom", Minimap):SetFrameStrata("LOW"):EnableMouse(false):ALL():SetScript("OnMouseWheel", function(self, delta)
    ...
    if ( delta > 0 ) then
        if Minimap.ZoomIn:IsEnabled() then Minimap.ZoomIn:Click(); end
    elseif ( delta < 0 ) then
        if Minimap.ZoomOut:IsEnabled() then Minimap.ZoomOut:Click(); end
    end
end):un();
...
function U1_MMBCreateCoordsButton()
    local btn = CreateFrame("Frame","MinimapCoordsButton",UIParent,"BackdropTemplate")

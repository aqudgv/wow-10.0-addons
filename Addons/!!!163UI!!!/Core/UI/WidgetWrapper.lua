...
local num = #SupportedTypes;
for i=1, num do
    local u = strupper(SupportedTypes[i]);
    local l = strlower(SupportedTypes[i]);
    SupportedTypes[#SupportedTypes + 1] = u;
    SupportedTypes[#SupportedTypes + 1] = l;
    SupportedTypes[SupportedTypes[i]] = true
    SupportedTypes[u] = true
    SupportedTypes[l] = true
end
...
lib_meta.__index = function(lib, key)
    ...
    if SupportedTypes[key] then
        func = function(self, name, parent, template)
            if parent and getmetatable(parent)==wrapper_meta then
                parent = parent._F
            end
            local frame = CreateFrame(key, name, parent, template and (template .. ",BackdropTemplate") or "BackdropTemplate")
            return self:Wrap(frame)
        end
        lib[key] = func
        lib[strupper(key)] = func
        lib[strlower(key)] = func
        return func
...
wrapper_meta.__index = function(wrapper, key)
    ...
    if SupportedTypes[key] then
        func = function(self, name, template, parentKey)
            ...
            for key, method in next, BackdropTemplateMixin do frame[key] = method end
        ...
        wrapper_meta[strupper(key)] = func
        wrapper_meta[strlower(key)] = func
        return func
    end

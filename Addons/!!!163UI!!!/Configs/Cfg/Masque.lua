...
U1RegisterAddon("Masque", {
...

	{
		...
		callback = function(cfg, v, loading)
			if MainMenuBar.EndCaps then
				MainMenuBar.EndCaps:SetShown(not v);
			else
	...
	},
	{
		...
		default = 1,
		callback = function(cfg, v, loading)
			(MainMenuBarArtFrameBackground or MainMenuBar.BorderArt):SetShown(not v);
...
CoreDependCall("Masque", function()
	CoreLeaveCombatCall("cfgmasque_blizz", nil, function()
		...
		AddButtonToGroup('StanceButton%d', NUM_STANCE_SLOTS or 12, '姿态动作条')

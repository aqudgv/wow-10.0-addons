...
U1RegisterAddon("Parrot", {
...

    {
        text = "配置选项",
        tip = "快捷命令`/parrot",
        callback = function(cfg, v, loading) SlashCmdList.PARROT() end,
    },
...

    {
        ...
        default = true,
        getvalue = function() return Parrot:GetModule("CombatEvents").db.profile.hideThrottleText end,
        callback = function(info, v, loading)
            Parrot:GetModule("CombatEvents").db.profile.hideThrottleText = v
        end
    },

    {
        ...
        var = "shortenAmount",
        default = true,
        getvalue = function() return Parrot:GetModule("CombatEvents").db.profile.shortenAmount end,
        callback = function(info, v, loading)
            Parrot:GetModule("CombatEvents").db.profile.shortenAmount = v
        end,

        {
            ...
            default = 2,
            callback = function(info, v, loading)
                Parrot:GetModule("CombatEvents").db.profile.shortenAmountType = v
            end,
        }
    },

    {
        ...
        cols = 2,
        getvalue = function() return Parrot.db:GetCurrentProfile() end,
        callback = function(info, v, loading)
            if loading then
                hooksecurefunc(Parrot.db, "ResetProfile", function(self, ...)
                    if Profiles[self:GetCurrentProfile()] then
                        CreateProfileAndApply(self:GetCurrentProfile())
                    end
                end)

                local tbl = Parrot.db:GetProfiles()
                local found
...
Profiles = {
    [PROFILE_SIMPLE] = {
        ...
        CombatEvents = {
            ...
            --/save Parrot.modules.CombatEvents.db.profile.Outgoing
            Outgoing = {
...
function CreateProfileAndApply(name)
    Parrot.db:SetProfile(name)
    for k,v in pairs(Profiles[name]) do
        if type(v) == "table" then
            deepmix(Parrot.db.children[k].profile, v)
        end
    end
    Parrot.db:SetProfile(name)
end
...
function P:SaveProfile(prof)
    if(prof.config.u1dbconfigs) then
        prof.u1dbconfigs = copyTable(U1DBA.configs, prof.u1dbconfigs)
        prof.u1dbframes = copyTable(U1DBA.frames, prof.u1dbframes)
        prof.u1dbignoreList = copyTable(U1DBA.ignoreList, prof.u1dbignoreList)
        prof.u1dbcollectList = copyTable(U1DBA.collectList, prof.u1dbcollectList)
    else
    ...
    if(prof.config.u1dbaddons) then
        prof.u1dbaddons = copyTable(U1DBA.addons, prof.u1dbaddons)
    else
        prof.u1dbaddons = nil
    end
...
function P:LoadProfile(prof, opts)
    ...
    if(opts.u1dbconfigs and prof.u1dbconfigs) then
        U1DBA.configs = copyTable(prof.u1dbconfigs, U1DBA.configs)
        U1DBA.frames = copyTable(prof.u1dbframes, U1DBA.frames)
        U1DBA.ignoreList = copyTable(prof.u1dbignoreList, U1DBA.ignoreList)
        U1DBA.collectList = copyTable(prof.u1dbcollectList, U1DBA.collectList)
    end

    if(opts.u1dbaddons and prof.u1dbaddons) then
        U1DBA.addons = copyTable(prof.u1dbaddons, U1DBA.addons)
        self:EnableOrDisableAddOn(U1DBA.addons)
    end
end
...
eventFrame:SetScript('OnEvent', function(self, event)
    ...
    elseif event == 'PLAYER_LOGIN' then
        ...
        local last_time = U1DBA.last_logout_time
    ...
    elseif event == 'PLAYER_LOGOUT' then
        U1DBA.last_logout_time = reloading and "reload" or time()

...
--]====]
local CAddon, U1 = ...;
local _C_CORE_PATH_TEXTURE = [[Interface\AddOns\!!!163UI!!!\Textures\flat]];
...
local LibSerializer = LibStub("AceSerializer-3.0");
if LibSerializer == nil then
	print("Error: |cffff0000Missing Lib 'AceSerializer-3.0'|r");
	return false;
end
LibSerializer:Embed(WExtern);
--	str = @Serialize(value);
...
-->		Dev Purpose Field
local is_dev = select(2, GetAddOnInfo("!!!!core!!!!")) ~= nil;
local pcall = is_dev and function(func, ...) return true, func(...); end or pcall;
local FDebug = is_dev and print or function() end;
...
local TAddonVariableMeta = {
	['!!!163ui!!!'] = {
		FGet = function(var)
			if U1DBG ~= nil then
				if U1DBG.__useAccountGlobalSetting and
				U1DBG.__useAccountGlobalSettingChars ~= nil and U1DBG.__useAccountGlobalSettingChars[CGUID] and
				U1DBG.__useAccountGlobalSettingBlackList ~= nil and not U1DBG.__useAccountGlobalSettingBlackList[CGUID] then
					return U1DBG.__globalSetting;
				else
					return U1DB;
				end
			end
...
function WExtern:FGet()
	...
	for index = 1, #TBlzOptionKeys do
		...
		elseif type(key) == 'table' then
			...
			print("|124cffff0000Invalid Option Key ", index, key);
		end
	end
...
local function FConfirmButtonOnClick(self, button)
	...
	elseif self.var == 'okay' then
		if InCombatLockdown() then
			print("|cffff0000战斗状态中，请脱离战斗后继续|r")
		else
			WExtern.WConfirm:Hide();
			WExtern:FApply();
		end
...
function WExtern:FConfirmApply(TData)
	...
	if WConfirm == nil then
		...
		FMakeCheckButton(WConfirm, FConfirmCheckButtonOnClick, 'addon_variable', "插件设置", nil, nil, "LEFT", WConfirm, "TOPLEFT", 24, -120);
		FMakeButton(WConfirm, FConfirmButtonOnClick, 'black', "选择插件", 90, nil, nil, "LEFT", WConfirm, "TOPLEFT", 126, -120);
...
-->		External
U1.ShowExern = function()
	WExtern:FShowGUI();
end
U1.ToggleExtern = function()

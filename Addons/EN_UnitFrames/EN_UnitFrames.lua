...
function EUF_Options_Update(oOptionId, oValue, notbool)
	if not EUF_CurrentOptions then
		return;
	end
	...
	if value ~= 1 and not notbool then
		value = 0;
	end
   ...
   elseif (optionId == "PLAYERFRM") then
	...
	elseif (optionId == "PARTYSCALE") then
		if EUF_FrameScale then
			EUF_FrameScale('party', value);
		end
...
function EUF_FormatNumericValue(value)
    if(EUF_CurrentOptions and EUF_CurrentOptions['NUMBERFORMAT'] == 1) then
		...
		elseif value <= 99999999 then
			value = value/1e4
			return (value - value % 1.0) .. "w";
		else
			value = value/1e8
			return (value - value % 1.0) .. "亿"
        end
    else -- 2
        if value <= 9999 then
            return value
...
frame:SetScript("OnEvent", function()
	if __ntodo > 0 then
		while __ntodo > 0 do
			local todo = __todo[__ntodo];
			__ntodo = __ntodo - 1;
			todo[1]:SetScale(todo[2]);
		end
	end
end);
function EUF_FrameScale(which, val)
	local _combat = InCombatLockdown();
	if which == 'player' then
		if _combat then
			__ntodo = __ntodo + 1;
			__todo[__ntodo] = { PlayerFrame, val, };
		else
	...
	elseif which == 'target' then
		if _combat then
			__ntodo = __ntodo + 1;
			__todo[__ntodo] = { TargetFrame, val, };
		else
			TargetFrame:SetScale(val);
		end
	elseif which == 'party' then
		for _index = 1, MAX_PARTY_MEMBERS do
			local _frame = getglobal("PartyMemberFrame" .. _index);
			if _combat then
				__ntodo = __ntodo + 1;
				__todo[__ntodo] = { _frame, val, };
			else
				_frame:SetScale(val);
			end
		end

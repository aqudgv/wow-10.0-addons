...
PlayerFrameHealthBar:HookScript("OnValueChanged", function(self, value)
	...
	valueMax = valueMax - valueMax % 1.0
...
function EUF_Fix920(Frame, To)
	local mb = To.manabar;
	local hb = To.healthbar;
	local tf = To.textureFrame;
	local maxFrameLevel = max(To:GetFrameLevel(), mb and mb:GetFrameLevel() or 0, hb and hb:GetFrameLevel() or 0, tf and tf:GetFrameLevel() or 0);
	if Frame:GetFrameLevel() < maxFrameLevel then

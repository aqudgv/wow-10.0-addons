...
function DWChatFrame:OnInitialize() 
	...
	CoreUIEnableTooltip(DWCPullButton, "开怪提示");
	DWCPullButtonText:SetText(L["Pull"])
end
...
function DWChatFrame:Refresh() 
	...
	-- v={text=function() return L["Pull"] end, chatType="PULL", show=function() return true end,  index=0}
	-- local tabPull = createChatTab( v.text, v.chatType, v.show, v.index, 20);
	...
	DWCPullButton:ClearAllPoints();
	DWCPullButton:SetPoint("LEFT", dwChannelMuteButton,"RIGHT",1,0);
...
function DWC_RefreshPosition()
    if not hooked then
        ...
        hooksecurefunc(ChatFrame2EditBox, "SetPoint", function() if not refreshing then DWC_RefreshPosition() end end)
        hooksecurefunc(ChatFrame3EditBox, "SetPoint", function() if not refreshing then DWC_RefreshPosition() end end)
        hooksecurefunc(ChatFrame4EditBox, "SetPoint", function() if not refreshing then DWC_RefreshPosition() end end)
        hooksecurefunc(ChatFrame5EditBox, "SetPoint", function() if not refreshing then DWC_RefreshPosition() end end)
        hooksecurefunc(ChatFrame6EditBox, "SetPoint", function() if not refreshing then DWC_RefreshPosition() end end)
        hooksecurefunc(ChatFrame7EditBox, "SetPoint", function() if not refreshing then DWC_RefreshPosition() end end)
        hooksecurefunc(ChatFrame8EditBox, "SetPoint", function() if not refreshing then DWC_RefreshPosition() end end)
        hooksecurefunc(ChatFrame9EditBox, "SetPoint", function() if not refreshing then DWC_RefreshPosition() end end)
        hooksecurefunc(ChatFrame10EditBox, "SetPoint", function() if not refreshing then DWC_RefreshPosition() end end)
        hooked = true
    end
    refreshing = true
    --[[local xo, yo = -5, -2

...
local SetWhoToUI = SetWhoToUI or C_FriendList.SetWhoToUi
...
function DuowanChat:GetModuleEnabled(module)
    return db.modules and db.modules[module] 
end 

function DuowanChat:SetModuleEnabled(module, value)
    local old = db.modules == nil and "_non_equal_28347984379987438927589375893275382798537_你要跟我相同我跟你姓" or db.modules[module]
...
local _bfchannel_filter = function(cf, event, msg, sender, lang, channelStr,
    ...
    if channelName == L['BigFootChannel'] or channelName == "世界" or channelName == "世" then
    -- if(channelName:find(L['BigFootChannel'])) then
        -- if(DuowanChat.tekdebug) then
...
function PullButton_OnClick(self, button)
	if (SlashCmdList["DEADLYBOSSMODS"]) then
		if (button == "LeftButton") then
			SlashCmdList["DEADLYBOSSMODS"]("PULL 7")
			if (UnitIsGroupLeader("player") or UnitIsGroupAssistant("player")) then
				SendChatMessage(">> 有爱提醒：7秒后开始战斗 <<", "RAID_WARNING")
			else
				SendChatMessage(">> 有爱提醒：7秒后开始战斗 <<", "YELL")
			end
		else
			SlashCmdList["DEADLYBOSSMODS"]("PULL 0")
			if (UnitIsGroupLeader("player") or UnitIsGroupAssistant("player")) then
				SendChatMessage("<< 取消倒数 >>", "RAID_WARNING")
			else
				SendChatMessage("<< 取消倒数 >>", "YELL")
			end
		end
	else
		print("开怪倒数的功能需要载入 DBM") 
	end
end

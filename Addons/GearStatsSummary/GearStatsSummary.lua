...
-- 2=布甲,3=皮甲,4=鎖甲,5=鎧甲
local sequence_RATING_SUMMARY_ENCHANTABLES = {
	INVSLOT_MAINHAND, INVSLOT_OFFHAND, INVSLOT_CHEST, INVSLOT_HAND, INVSLOT_WRIST, INVSLOT_FEET, INVSLOT_FINGER1, INVSLOT_FINGER2, INVSLOT_BACK
}
...
function GearStatsSummary_Sum(inspecting, classID, specID)
	...
	for i=INVSLOT_FIRST_EQUIPPED, INVSLOT_LAST_EQUIPPED do --zhengruiw02
		...
		if (link) and i ~= INVSLOT_BODY and i ~= INVSLOT_TABARD then
			...
			local shortname = RATING_SUMMARY_ENCHANTABLES[i]
			if shortname then
				if i ~= INVSLOT_OFFHAND or ItemEquipLoc ~= "INVTYPE_SHIELD" then
	...
	for i = 1, #sequence_RATING_SUMMARY_ENCHANTABLES do
		local slot = sequence_RATING_SUMMARY_ENCHANTABLES[i]
		local shortname = enchantTbl[slot];
		if shortname then
			sum["EnchantMissing"] = sum["EnchantMissing"] .. shortname
		end
	end

...
local function OnShow(self, ...)
	...
	if self:GetName() == "ContainerFrameCombinedBags" then
		local def = self.settings.default
		if def then
			self:ClearAllPoints()
			self:SetPoint(def.point,def.relativeTo, def.relativePoint, def.xOfs,def.yOfs)
		end
		return
	end
...
local function OnDragStop(self)
	...
	if frameToMove:GetName() == "ContainerFrameCombinedBags" then
...
local function SetMoveHandler(frameToMove, handler)
	...
	if frameToMove:GetName() == "ContainerFrameCombinedBags" then
		frameToMove:HookScript("OnHide", OnHide)
	end
...
local function OnEvent(self, event, arg1, arg2)
	...
	if arg1 == "BlizzMove" then
		...
		-- 10.0
		SetMoveHandler(ContainerFrameCombinedBags)
		if ProfessionsFrame then
			SetMoveHandler(ProfessionsFrame);
		end
	...
	elseif arg1 == "Blizzard_AchievementUI" then
		SetMoveHandler(AchievementFrame, AchievementFrame.Header)
	elseif arg1 == "Blizzard_ClassTalentUI" then
		SetMoveHandler(ClassTalentFrame)
	...
	elseif arg1 == "Blizzard_AuctionHouseUI" then
		SetMoveHandler(AuctionHouseFrame)
	...
	elseif arg1 == "Blizzard_AzeriteUI" then
		SetMoveHandler(AzeriteEmpoweredItemUI);
	elseif arg1 == "Blizzard_AzeriteEssenceUI" then
		SetMoveHandler(AzeriteEssenceUI);
